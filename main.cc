/*
	Author: Junzhao Yang (FizzyDavid)
	Email: fizzydavid@qq.com
*/
#include<bits/stdc++.h>
using namespace std;
#define FF first
...
const int M = 64; //maximum size for node
...
private:
	...
	public:
		...
		int sum;  //the total number of keys in the subtree
		...
		int find_pos_from(const int &key, int pos) const
		{
			const int *it, *end = k+sz;
	...
	int get_cntsum(int x, int l, int r)
	{
		if (l>r) return 0;
		int ret = 0;
		if (r-l+1<=a[x].sz/2)
		{
			for (int i=l; i<=r; i++) ret += a[a[x].p[i]].sum;
		}
		else
		{
			ret = a[x].sum;
			for (int i=0; i<l; i++) ret -= a[a[x].p[i]].sum;	
			for (int i=r+1; i<=a[x].sz; i++) ret -= a[a[x].p[i]].sum;
		}
		return ret;
	}
	...
	int query(int x, int l, int r)
	{
		...
		else
		{
			...
			int ret = get_cntsum(x, pl+1, pr-1);
